...
FIRST_RDS_REPLICA_NAME="first-read-replica"
SECOND_RDS_REPLICA_NAME="second-read-replica"
...
FIRST_REPLICA_INSTANCE_DNS_NAME=$(aws rds describe-db-instances --db-instance-identifier "${FIRST_RDS_REPLICA_NAME}" | jq '.["DBInstances"][0].Endpoint["Address"]')
SECOND_REPLICA_INSTANCE_DNS_NAME=$(aws rds describe-db-instances --db-instance-identifier "${SECOND_RDS_REPLICA_NAME}" | jq '.["DBInstances"][0].Endpoint["Address"]')
...
cat <<EOF > record-sets.json
{
            ...
            "Action": "CREATE",
                        "ResourceRecordSet": {
                                    ...
                                    "TTL": 300,
                                    "Weight": 100,
                                 "ResourceRecords": [{ "Value": ${FIRST_REPLICA_INSTANCE_DNS_NAME}}]
                        }
            },
            {
            "Action": "CREATE",
                        "ResourceRecordSet": {
                                    "Name": "instance.${ROUTE53_HOSTED_ZONE}",
                                    "Type": "CNAME",
                                    "SetIdentifier": "replica2",
                                    "TTL": 300,
                                    "Weight": 100,
                                 "ResourceRecords": [{ "Value": ${SECOND_REPLICA_INSTANCE_DNS_NAME}}]
                        }
            }]
}
EOF
